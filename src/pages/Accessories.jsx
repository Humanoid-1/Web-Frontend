...
function AccessoriesPage() {
  ...
  const categories = [
    "Mouse", "Keyboard", "Charger", "Docking Station", "Headphones", "Speakers",
    "Pendrive", "Webcam", "USB Flash Drive", "Cooling Pad", "Microphone", "External Hard Drive"
  ];

  const brands = [
    "All","Dell","HP","Lenovo","Asus","Acer","Apple","MSI","Samsung","LG","Huawei",
    "Google","Microsoft","Sony","Logitech","Corsair","Razer"
  ];
  ...
  return (
    <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px" }}>
      ...
      <div style={{ marginBottom: "20px", textAlign: "center" }}>
        <label htmlFor="brand" style={{ marginRight: "10px", fontWeight: "600" }}>Filter by Brand:</label>
        <select
          id="brand"
          value={brand}
      ...
      {/* Category buttons */}
      <div style={{ marginBottom: 32, display: "flex", flexWrap: "wrap", gap: 14, justifyContent: "center" }}>
        {categories.map((c) => (
          <button
            key={c}
            ...
            style={{
              padding: "10px 22px", border: "none", borderRadius: "20px",
  ...
  {accessories.length > 0 ? (
    accessories.map((item) => {
      ...
      return (
        <Link
          key={item._id}
          to={`/accessories/${category}/${item._id}`}
      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: "30px", gap: "6px", flexWrap: "wrap" }}>
        <button onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
          disabled={currentPage === 1} style={{
            padding: "6px 12px", border: "1px solid #ccc", borderRadius: "6px",
            background: currentPage === 1 ? "#eee" : "white", cursor: currentPage === 1 ? "not-allowed" : "pointer"
          }}>Prev</button>
        ...
        {getPageNumbers().map((page, index) =>
          page === "left-ellipsis" || page === "right-ellipsis" ? (
            ...
            <button key={index} onClick={() => setCurrentPage(page)} style={{
              padding: "6px 12px", border: "1px solid #ccc", borderRadius: "6px",
